...
async def main():
    ...
    if items:
        ...
        # ----------------------------------------------------
        # 🗑️ REMOVER COLUNA 'acompanha' (NOVO)
        # ----------------------------------------------------
        if 'acompanha' in df.columns:
            df = df.drop(columns=['acompanha','categorias'])
            logger.info("🗑️ Colunas removidas com sucesso.")
        # ----------------------------------------------------
        ...
        filename_local = "product_derivation.csv"
