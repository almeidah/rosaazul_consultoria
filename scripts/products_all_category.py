...
# -------------------------------
# 5️⃣ Buscar todos os produtos + suas características
# -------------------------------
async def fetch_all_produtos():
    async with aiohttp.ClientSession() as session:
        ...
        # 2. Puxa as características dinâmicas de cada produto pai
        if all_items:
            logger.info(f"⚡ Buscando características dinâmicas de {len(all_items)} produtos pai...")
            chars_results = []
            chunk_size = 300 # Lotes de 300 para não estourar o limite de tempo/conexão do Python
...
async def main():
    ...
    if items:
        ...
        filename_local = "products_all_category.csv"
